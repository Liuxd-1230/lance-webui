...
# 激活 venv
& $venvActivate
$pyVenv = Join-Path $PSScriptRoot "venv\Scripts\python.exe"
...
if (-not (Test-Path $marker)) {
    ...
    # 升级 pip
    & $pyVenv -m pip install --upgrade pip -q 2>$null

    # 先试清华源
    & $pyVenv -m pip install -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple/ --trusted-host pypi.tuna.tsinghua.edu.cn
    if ($LASTEXITCODE -ne 0) {
...
# ── 配置 ──────────────────────────────────────────────
$host = if ($env:LANCE_HOST) { $env:LANCE_HOST } else { "0.0.0.0" }
...
if ($occupied) {
    $pid = $occupied[0].OwningProcess
    $proc = Get-Process -Id $pid -ErrorAction SilentlyContinue
    Write-Host "[WARN] 端口 $port 已被占用 (PID: $pid $($proc.ProcessName))" -ForegroundColor DarkYellow
    $kill = Read-Host "是否终止该进程? (y/N)"
    if ($kill -eq "y" -or $kill -eq "Y") {
        Stop-Process -Id $pid -Force
        Start-Sleep -Seconds 1
        Write-Host "[INFO] 已终止进程 $pid" -ForegroundColor Green
    } else {
...
# 启动 uvicorn
& $pyVenv -m uvicorn backend.main:app --host $host --port $port --reload
